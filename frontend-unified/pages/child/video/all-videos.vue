...
// 加载视频列表
const loadVideos = async (refresh = false, page = null) => {
  ...
  try {
    ...
    if (response && response.data) {
      ...
      console.log('视频列表加载成功，总数据：', videos.value.length, '条，当前页：', currentPage.value, '总页数：', totalPages, '还有更多：', hasMore.value)
    } else {
...
const navigateToVideoPlayer = (video) => {
  ...
  // 跳转到视频播放页面
  uni.navigateTo({
    url: `/pages/child/video-detail?id=${video.id}&videoInfo=${videoInfoStr}`
  })
}
...
.header {
  ...
  left: 0;
  right: 0;
  background-color: #8b5cf6; /* 儿童端紫色主题 */
  z-index: 50;
}
