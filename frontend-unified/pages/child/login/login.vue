...
const handleLogin = async () => {
  ...
  try {
    ...
    // payload 是 base64 编码的 JSON，包含 aud（用户ID）
    try {
      const tokenParts = res.data.accessToken.split('.')
      if (tokenParts.length === 3) {
        // 解码 payload（第二部分）
        const payload = JSON.parse(atob(tokenParts[1]))
        const userId = payload.aud // JWT中 aud 字段存放用户ID
        if (userId) {
          uni.setStorageSync('userId', userId)
          uni.setStorageSync('currentUserId', userId)
          console.log('已从token中解析用户ID：', userId)
        }
      }
    } catch (error) {
      console.warn('从token中解析用户ID失败，将在首页重新获取', error)
    }
...
const goToRegister = () => {
  console.log('跳转到孩子端注册页面')
  uni.navigateTo({
    url: '/pages/child/register',
    success: () => {
...
.confirm-btn:active {
  transform: translateY(2rpx);
  box-shadow: 0 4rpx 8rpx rgba(139, 92, 246, 0.15);
}
</style>
