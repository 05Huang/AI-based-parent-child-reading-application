  ...
  <view class="container">
    ...
    >
      ...
      <view class="comments-list">
        <!-- 暂无评论提示 -->
        <view v-if="!loading && comments.length === 0" class="empty-comments">
          <text class="fas fa-comment-slash"></text>
  ...
  if (pages.length > 1) {
    // 如果有上一页，直接返回
      ...
      delta: 1,
        ...
        if (contentId.value) {
          uni.redirectTo({
            url: `/pages/child/article-detail?id=${contentId.value}`
          })
        } else {
          uni.reLaunch({
...
const viewAllReplies = (comment) => {
  // 孩子端暂时在当前页面展开所有回复
  // TODO: 后续可以创建独立的回复列表页面
  console.log('查看评论的所有回复：', comment.id)
  uni.showToast({
    title: '功能开发中',
    icon: 'none'
  })
