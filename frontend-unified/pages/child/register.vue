...
// 兴趣标签
const interests = ref([
  { name: '冒险故事', active: false },
  { name: '奇幻魔法', active: false },
  { name: '科学知识', active: false },
  { name: '历史故事', active: false },
  { name: '童话世界', active: false },
  { name: '动物百科', active: false }
])
...
const handleRegister = async () => {
  ...
  try {
    ...
    // 生成用户名（使用昵称+随机数）
    const username = `child_${Math.floor(Math.random() * 10000)}`
    ...
    // 构建请求数据
      ...
      interests: interestsString // 将兴趣爱好数组转为逗号分隔的字符串
        ...
        // 打印请求数据
        console.log('发送孩子端注册请求数据：', JSON.stringify(requestData, null, 2))
        
    // 调用后端注册接口
        const res = await request.post('/api/user/register-by-phone', requestData)
       
        console.log('孩子端注册响应：', res)
      ...
      uni.reLaunch({
        ...
        if (error.response && error.response.data) {
          ...
          if (responseData.code === 400) {
            ...
            } else if (responseData.message.includes('已存在')) {
...
.avatar-img {
  ...
  height: 100%;
  border-radius: 999px;
  box-shadow: 0 8rpx 32rpx rgba(139, 92, 246, 0.2);
...
@keyframes cameraPulse {
  ...
  50% {
    transform: scale(1.1);
    box-shadow: 0 8rpx 28rpx rgba(139, 92, 246, 0.6);
  }
}
...
.input-box:hover {
  box-shadow: 0 8rpx 30rpx rgba(139, 92, 246, 0.1);
