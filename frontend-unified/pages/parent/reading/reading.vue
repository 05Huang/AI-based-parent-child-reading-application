...
const getReadBooksStorageKey = () => {
  if (!currentUser.value?.id) return null
  return `readBooks:${currentUser.value.id}`
}
...
onLoad(async (option) => {
  ...
  if (option.id) {
    ...
    readingTimer.value = setInterval(() => {
      const now = Date.now()
      const durationSeconds = Math.floor((now - readingStartTime.value) / 1000)
      const minutes = Math.floor(durationSeconds / 60)
      if (minutes > 0) {
        console.log(`已阅读 ${minutes} 分钟`)
      }
    }, 60000)
  } else {
    error.value = '文章ID不存在'
  ...
  if (readingStartTime.value && currentUser.value?.id && article.value?.id) {
    ...
    if (duration >= 3) {
      try {
        ...
        const storageKey = getReadBooksStorageKey()
        if (storageKey) {
          const existing = uni.getStorageSync(storageKey)
          const readBookIds = Array.isArray(existing) ? existing : []
          if (!readBookIds.includes(article.value.id)) {
            readBookIds.push(article.value.id)
            uni.setStorageSync(storageKey, readBookIds)
            console.log(`[阅读页] 阅读本数+1（本地统计），当前共 ${readBookIds.length} 本`)
          }
        }
        eventBus.emit(EVENTS.USER_STATS_UPDATED, {
