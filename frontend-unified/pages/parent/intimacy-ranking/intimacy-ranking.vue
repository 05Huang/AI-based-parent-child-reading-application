  ...
  <view class="container">
    ...
    <view class="title-section">
      <view class="title-card">
        <view class="ranking-header-title">亲密度排行榜</view>
        <view class="ranking-header-subtitle">提升您与孩子的亲密度，赢在家庭教育的起跑线！</view>
        ...
        <view class="tips-list">
          <view class="tip-item">
            ...
            <view class="tip-content">
              <view class="tip-title">每日共读</view>
              <view class="tip-desc">每天坚持与孩子共读15分钟，对话交流可提升亲密度。</view>
            </view>
          </view>
          <view class="tip-item">
            ...
            <view class="tip-content">
              <view class="tip-title">互动交流</view>
              <view class="tip-desc">对孩子的问题及时回应，与他们多进行思考型交流。</view>
            </view>
          </view>
          <view class="tip-item">
            ...
            <view class="tip-content">
              <view class="tip-title">积极参与</view>
              <view class="tip-desc">积极参与孩子的学习活动和游戏，提高互动质量。</view>
            </view>
          </view>
        </view>
      </view>
...
const loadGlobalRankingData = async () => {
  try {
    ...
    if (response && response.data && response.data.ranking) {
      ...
      console.log('全网排行榜数据处理完成，共 ' + globalRankings.value.length + ' 条记录')
    }
  } catch (error) {
...
const loadFamilyRankingData = async () => {
  try {
    ...
    if (response && response.data) {
      ...
      // 输出计算日志
      console.log('【亲密度计算日志】当前用户全局信息:', {
          userId: currentUser.value.id,
          nickname: currentUser.value.nickname,
          globalRank: response.data.globalRank,
          intimacyPercentage: response.data.intimacyPercentage,
          familyMemberCount: response.data.ranking ? response.data.ranking.length : 0
      })
...
const loadIntimacyTrend = async () => {
  try {
    ...
    console.log('开始加载亲密度趋势图数据，用户ID：', currentUser.value.id)
    
    // 调用后端接口
    const days = currentChartTimeRange.value === '近7天' ? 7 : 30
    const response = await intimacyApi.getIntimacyTrend(currentUser.value.id, days)
    
    if (response && response.data) {
        console.log('获取亲密度趋势数据成功：', response.data)
        ...
        if (!intimacyChart) {
          ...
          if (chartDom) {
            intimacyChart = echarts.init(chartDom)
             ...
             console.error('无法找到图表容器 #intimacyChart')
             return
          }
        }
...
onMounted(async () => {
  ...
  // 总是加载家庭排行数据以获取用户的个人全局排名信息（解决个人排名不显示的问题）
  await loadFamilyRankingData()
