...
const loadFeaturedVideo = async (shuffle = false) => {
  try {
    if (currentUser.value && currentUser.value.id) {
      console.log('开始加载推荐视频（Python 推荐系统）');
      const response = await recommendationApi.getVideoRecommendations(currentUser.value.id, 1, shuffle);
      if (response && Array.isArray(response.data) && response.data.length > 0) {
        const video = response.data.find(item => item && item.type === 2) || response.data[0];
        if (video && video.id) {
          featuredVideo.value = video;
          console.log('推荐视频加载成功（Python 推荐）：', featuredVideo.value.title || featuredVideo.value.id);
          return;
        }
      }
      console.warn('推荐系统未返回有效推荐视频，使用随机方案');
    } else {
      console.warn('用户未登录，使用随机推荐视频方案');
    }
  } catch (error) {
  ...
  try {
    ...
    console.error('加载推荐视频失败（随机方案）：', error);
  }
};
...
const loadHotVideos = async (shuffle = false) => {
  try {
    if (currentUser.value && currentUser.value.id) {
      console.log('开始加载热门视频（Python 推荐系统）');
      const response = await recommendationApi.getHotVideos(currentUser.value.id, 10, shuffle);
      if (response && Array.isArray(response.data) && response.data.length > 0) {
        ...
        if (hotVideos.value.length > 0) {
          console.log('热门视频加载成功（Python 推荐），共', hotVideos.value.length, '个');
          return;
        }
      }
      console.warn('热门视频推荐系统未返回有效数据，使用随机方案');
    } else {
      console.warn('用户未登录，使用随机热门视频方案');
    }
  } catch (error) {
  ...
  try {
    ...
    console.error('加载热门视频失败（随机方案）：', error);
  }
};
...
const loadRecommendedVideos = async (shuffle = false) => {
  try {
    if (currentUser.value && currentUser.value.id) {
      console.log('开始加载为你推荐视频（Python 推荐系统）');
      const response = await recommendationApi.getVideoRecommendations(currentUser.value.id, 10, shuffle);
      if (response && Array.isArray(response.data) && response.data.length > 0) {
        const existingIds = getExistingVideoIds();
        const candidates = response.data.filter(item => item && item.type === 2 && item.id && !existingIds.has(item.id));
        const shuffled = candidates.sort(() => Math.random() - 0.5);
        ...
        if (recommendedVideos.value.length > 0) {
          console.log('为你推荐视频加载成功（Python 推荐），共', recommendedVideos.value.length, '个');
          return;
        }
      }
      console.warn('为你推荐视频系统未返回有效数据，使用随机方案');
    } else {
      console.warn('用户未登录，使用随机为你推荐方案');
    }
  } catch (error) {
    console.error('加载为你推荐视频失败（Python 推荐），使用随机方案：', error);
  }
  ...
  try {
    ...
    console.error('加载推荐视频列表失败（随机方案）：', error);
  }
};
...
const loadVideosForCurrentTab = async (shuffle = false) => {
  ...
  categoryVideoPool.value = [];
  await loadFeaturedVideo(shuffle);
  await loadHotVideos(shuffle);
  await loadRecommendedVideos(shuffle);
};
