  ...
  <view class="container">
    ...
    <scroll-view scroll-y="true" class="content-area">
      ...
      <view class="video-description" v-if="videoInfo.content">
        ...
        <text class="description-text">{{ videoDescriptionText }}</text>
      </view>
...
import { videoApi, commentApi, likeApi, userApi, friendApi, recommendationApi } from '../../../utils/api.js';
...
const videoDescriptionText = computed(() => {
  const raw = videoInfo.value?.content || videoInfo.value?.description || ''
  ...
  try {
    ...
    if (firstResponse.code === 200 && firstResponse.data) {
      ...
      if (response.data && response.data.records && response.data.records.length > 0) {
        const videos = response.data.records.filter(item => item.type === 2 && item.id && item.id != videoId.value);
        const shuffled = shuffleVideos(videos);
        relatedVideos.value = shuffled.slice(0, 3);
        console.log('相关视频加载成功（随机降级），共', relatedVideos.value.length, '个');
      }
    }
  } catch (error) {
    console.error('加载相关视频异常（随机降级）：', error);
  }
};
...
const loadRelatedVideos = async () => {
  try {
    if (currentUser.value && currentUser.value.id) {
      console.log('开始加载相关视频（Python 推荐系统）');
      const response = await recommendationApi.getVideoRecommendations(currentUser.value.id, 20, true);
      if (response && Array.isArray(response.data) && response.data.length > 0) {
        const filtered = response.data.filter(item => item && item.type === 2 && item.id && item.id != videoId.value);
        const shuffled = shuffleVideos(filtered);
        relatedVideos.value = shuffled.slice(0, 3);
        if (relatedVideos.value.length > 0) {
          console.log('相关视频加载成功（Python 推荐），共', relatedVideos.value.length, '个');
          return;
        }
      }
      console.warn('推荐系统未返回有效相关视频，使用降级方案');
    } else {
      console.warn('用户未登录，使用降级方案加载相关视频');
    }
  } catch (error) {
    console.error('加载相关视频异常（Python 推荐），使用降级方案：', error);
  }
