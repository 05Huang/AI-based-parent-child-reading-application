    ...
    >
      ...
      <swiper class="banner-swiper" 
        ...
        <swiper-item v-for="item in swiperList" :key="item.id" class="swiper-item">
          <view class="banner-item" @click="navigateToReading(item)">
            <image :src="item.coverUrl" mode="aspectFill" class="banner-image"></image>
          ...
          >
            ...
            <view class="hot-book-info">
              <view class="hot-book-content">
                ...
                <text class="hot-book-author">作者：{{ item.creatorName || '阅读小助手' }}</text>
              </view>
              <view class="hot-book-footer">
                ...
                <view class="hot-book-stats">
                  <view class="stat-item" @click.stop="toggleLike(item, 'hot')">
                    <text class="fas fa-heart stat-icon" :class="{ 'liked': item.isLiked }"></text>
...
// 加载首页数据
const loadHomeData = async (shuffle = false) => {
  try {
    loading.value = true
    console.log('开始加载首页数据...')
    
    // 并行请求轮播图、推荐内容和热门内容
    const [swiperResult, recommendedResult, hotResult] = await Promise.allSettled([
      loadSwiperData(),
      loadRecommendedContents(shuffle),
      loadHotContents(shuffle)
    ])
    
    if (swiperResult.status === 'rejected') {
      console.error('加载轮播图失败：', swiperResult.reason)
    }
...
const loadRecommendedContents = async (shuffle = false) => {
  try {
    console.log('======================================')
...
const loadHotContents = async (shuffle = false) => {
  try {
    console.log('======================================')
...
const loadSwiperData = async () => {
  try {
    ...
    const params = {
      ...
      status: 1,
      type: 1, // 文章
      sortField: 'view_count', // 按浏览量排序
      sortOrder: 'desc'
    }
    
    const response = await contentApi.getContentPage(params)
    if (response && response.data && response.data.records) {
      swiperList.value = response.data.records
      console.log('轮播图数据加载完成', swiperList.value.length)
    }
  } catch (error) {
