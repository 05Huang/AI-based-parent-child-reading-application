...
class AdvancedSimilarityCalculator:
    ...
    @staticmethod
    def normalized_cosine_similarity(vec1: np.ndarray, vec2: np.ndarray) -> float:
        """归一化余弦相似度"""
        # L2归一化
    ...
    @staticmethod
    def angular_distance_similarity(vec1: np.ndarray, vec2: np.ndarray) -> float:
        """基于角度距离的相似度"""
        # 计算余弦相似度
    ...
    @staticmethod
    def combined_similarity(vec1: np.ndarray, vec2: np.ndarray, weights: Dict[str, float] = None) -> float:
        """组合多种相似度计算方法"""
        if weights is None:
            weights = {
                'cosine': 0.5,
        ...
        similarities = {
            'cosine': AdvancedSimilarityCalculator.normalized_cosine_similarity(vec1, vec2),
            'angular': AdvancedSimilarityCalculator.angular_distance_similarity(vec1, vec2),
            'euclidean': AdvancedSimilarityCalculator.euclidean_similarity(vec1, vec2),
            'manhattan': AdvancedSimilarityCalculator.manhattan_similarity(vec1, vec2)
        }

        # 加权平均
        combined_sim = sum(similarities[method] * weight
    ...
    def calculate_and_save_content_similarity(self, content_id: int, content_vector: List[float]) -> List[
        ...
        try:
            with conn.cursor() as cursor:
                ...
                """
                cursor.execute(sql, (content_id,))  # 确保这里的参数是一个元组

                similar_contents = []
                content_vector_np = np.array(content_vector)
                ...
                for row in cursor.fetchall():
                    ...
                    if other_vector is not None:
                        other_vector_np = np.array(other_vector)
                        ...
                        similarity = self.similarity_calculator.combined_similarity(
                            ...
                            weights={
                                'cosine': 0.4,  # 余弦相似度权重
                                'angular': 0.3,  # 角度相似度权重
                                'euclidean': 0.2,  # 欧氏距离相似度权重
                                'manhattan': 0.1  # 曼哈顿距离相似度权重
                            }
                        )

                        if similarity > 0.3:  # 相似度阈值
                            similar_contents.append((other_id, similarity))

                # 按相似度排序，取内容总数的一半
                similar_contents.sort(key=lambda x: x[1], reverse=True)
                top_similar = similar_contents[:min(len(similar_contents), 100)]  # 最多保存100个

                # 批量保存到数据库
                if top_similar:
                    values = []
                    for sim_id, score in top_similar:
                        values.append((content_id, sim_id, score, score))
                    ...
                    """
                    cursor.executemany(sql_insert, values)  # 使用executemany进行批量插入

                conn.commit()
                logging.info(
                    f"Saved {len(top_similar)} similar contents for content {content_id} using advanced similarity")
                return top_similar
        ...
        if content_type == 1:
            ...
            if len(text_vector) > 0:
                ...
                return jsonify({
                    ...
                    'similarity_method': 'combined'  # 标明使用了组合相似度方法
                })
            else:
                return jsonify({
                    'success': False,
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("\nUsing Advanced Similarity Calculation Methods:")
    print("- Normalized Cosine Similarity (40%)")
    print("- Angular Distance Similarity (30%)")
    print("- Euclidean Distance Similarity (20%)")
    print("- Manhattan Distance Similarity (10%)")
    print("=" * 50)
