...
public class FileController {
     ...
     */
    @PostMapping("/upload")
    public Result<UploadImageVO> uploadFile(@RequestParam("file") MultipartFile file, @RequestParam(value = "type", defaultValue = "image") String type) {
        try {
            log.info("开始上传文件，类型：{}", type);
            UploadImageVO result = fileService.uploadImage(file);
